var area = Array.from(new Set(winter.map(d => d.Pickup_Community_Area)));
...
function createScatter2() {
  ...
  svg.append("text")
    ...
    .text('Miles vs Total Price in Winter');
  ...
  function onBrush(event) {
    ...
    svg.property('value', winter.filter(isBrushed)).dispatch('input');
  }
...
function init() {
  const winterData = createScatter2()
  const bar = createbarChart2();

  d3.select(winterData).on('input', () => {
    bar.update(winterData.value);
  });

  // intial state of bar chart
  bar.update(winterData.value);

  const tipsData = createScatter1()
  const bar2 = createbarChart1();
